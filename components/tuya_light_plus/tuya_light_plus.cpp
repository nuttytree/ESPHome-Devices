...
void TuyaLightPlus::setup()
{
  ...
  {
    this->subscribe_homeassistant_state(&TuyaLightPlus::on_day_night_changed_, this->day_night_sensor_.value());
  }
}
...
void TuyaLightPlus::set_default_brightness(float brightness)
{
  ...
  {
    this->state_->current_values.set_brightness(this->default_brightness_.value());
...
void TuyaLightPlus::handle_tuya_datapoint_(tuya::TuyaDatapoint datapoint)
{
  ...
  if (datapoint.id == *this->switch_id_)
  {
    ...
    {
      this->parent_->set_datapoint_value(*this->dimmer_id_, this->brightness_to_tuya_level_(this->default_brightness_.value()));
      this->state_->current_values.set_brightness(this->default_brightness_.value());
    }
  }
