...
* **current_sensor** (Optional, id): ID of a sensor reporting current draw in amps. Required when `enable_anomaly_detection` is enabled. When set, turning the switch on always publishes an on state and energizes the output, but the pump's runtime is only accumulated while current confirms the motor is actually running.
